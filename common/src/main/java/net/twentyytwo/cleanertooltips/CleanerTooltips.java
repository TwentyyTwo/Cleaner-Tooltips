...
public class CleanerTooltips {
    ...
    private static final Identifier DURABILITY_ICON = location("textures/gui/attribute/durability.png");
    private static final Identifier   DIGGING_SPEED = location("textures/gui/attribute/digging_speed.png");
    private static final Identifier          HIGHER = location("textures/gui/attribute/higher.png");
    private static final Identifier           LOWER = location("textures/gui/attribute/lower.png");
    ...
    private static MutableComponent durabilityFormatting(ItemStack stack) {
        ...
        return switch (config.durabilityStyle) {
            ...
            case null, default -> {
                var remain = Component.literal(String.valueOf(cur)).withColor(durabilityColor);
                var total = Component.literal(String.format(" / %s", max)).withColor(0x555555);
                yield !config.durabilityMaximum ? remain : remain.append(total);
            }
        };
    }
    ...
    private static int getDurabilityColor(ItemStack stack, int percentage) {
        ...
        return switch (config.colorMode) {
            case LINEAR -> {
                ...
                yield ARGB.linearLerp(delta, lower.getValue(), upper.getValue());
            }
    ...
    private static void blit(GuiGraphicsExtractor graphics, Identifier location, int x, int y, int size) {
        graphics.blit(RenderPipelines.GUI_TEXTURED, location, x, y, 0, 0, size, size, size, size);
    }
    ...
    public record IconAttributeTooltip(ItemStack stack,
        ...
        public static IconAttributeTooltip fromStack(ItemStack stack) {
            ...
            modifiers.modifiers().forEach((slot, entry) -> {
                ...
                Identifier texture = AttributeManager.getTexture(entry.attribute());
                if (texture != null) {
        ...
        private Identifier getSlotIcon(String slotGroup) {
